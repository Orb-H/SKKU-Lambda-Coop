      ...
      if (req.method === 'POST') {
          admin.auth().verifyIdToken(body.token)
              ...
              if (uid === '73cxqheH7nMwI2Gj91ojCEfm1j73') {    
                ...
                obj.result = "true"
                obj.data,push({success: "true"});
              } else {

                
                obj.result = "false"
                obj.data.push({success: "false"});
              }
              return uid;
          ...
          if (req.method === 'POST') {
              admin.auth().verifyIdToken(body.token)
                  ...
                  if (uid === '73cxqheH7nMwI2Gj91ojCEfm1j73') {
                      ...
                      let allgifticons = gifticonsRef.get()
                      .then(snapshot => {
                          snapshot.forEach(doc => { 
                            ...
                            // name , category1, category2가 일치하는 content가 있으면 count ++ 해준다, 
                            else{

                                for (var i = 0 ; i < Object.keys(obj.data).length ; i ++){
                                    if (obj.data.content[i].name == doc.data().name && obj.content[i].category1 == doc.data().category1 &&obj.content[i].category2 == doc.data().category2 ){
                                        temp= 1;
                                        temp2 =i;
                                        break; 
                                    }
                        ...
                        var glength = Object.keys(obj).length;
                        obj.data.push({length: glength});
                        obj.result = "true";
                      }).catch(err => {
                        obj.result = "false";
                      })
